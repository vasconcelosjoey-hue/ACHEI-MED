        ...
        <div className="flex-1 grid lg:grid-cols-4 gap-8">
          ...
          <section className="lg:col-span-3 h-full min-h-[500px]">
            {searchMode === 'LIST' ? (
              ...
              <div className="relative w-full h-full rounded-[3rem] overflow-hidden border border-slate-100 shadow-2xl bg-[#E5E7EB]">
                {/* Simulated Map Background */}
                <div 
                  className="absolute inset-0 grayscale-[0.2] contrast-[0.9] brightness-[1.05]"
                  style={{
                    backgroundImage: 'url("https://api.mapbox.com/styles/v1/mapbox/light-v10/static/-60.0150,-3.1050,13,0,0/800x600?access_token=pk.eyJ1IjoiZGVtbyIsImEiOiJjbDFvMXB6MG0wMDB3M2pueHl1N2V3YmI1In0.demo")',
                ...
                {filteredDoctors.map((doc) => (
                  <button 
                    ...
                    style={{
                      left: `${50 + (doc.lng + 60.0150) * 800}%`,
                      top: `${50 + (doc.lat + 3.1050) * -800}%`
                    }}
                    className={`absolute -translate-x-1/2 -translate-y-full p-1 rounded-full border-2 transition-all hover:scale-125 z-10 ${selectedDoc?.id === doc.id ? 'bg-deepAqua border-white scale-110 shadow-xl' : 'bg-white border-deepAqua shadow-md'}`}
                  >
                    <img src={doc.avatar} alt={doc.name} className="w-8 h-8 rounded-full" />
                ...
                {/* Map Doctor Card Overlay */}
                {selectedDoc && (
                  <div className="absolute bottom-6 left-6 right-6 lg:left-1/2 lg:-translate-x-1/2 lg:w-full lg:max-w-md animate-in slide-in-from-bottom-10 duration-500">
                    <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl flex gap-6 items-center relative overflow-hidden group">
                      <button 
                      ...
                      <img src={selectedDoc.avatar} className="w-24 h-24 rounded-3xl object-cover shadow-lg border-4 border-slate-50" alt={selectedDoc.name} />
                      <div className="flex-1">
                        <h4 className="text-xl font-display font-bold text-slate-900 leading-tight">{selectedDoc.name}</h4>
                        <p className="text-sm font-semibold text-deepAqua mb-1">{selectedDoc.specialty}</p>
                        <p className="text-[10px] text-slate-400 font-medium leading-relaxed mb-4 flex items-center gap-1.5">
                           <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                           {selectedDoc.address}
                        </p>
                        <button 
                          onClick={() => handleWhatsappBooking(selectedDoc)}
                          className="w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-3 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-green-200"
                        >
